...
import { createData, getcartProduct } from "../../redux/slice/cart";
...
const Details = () => {
  ...
  const handleAddData = async () => {
    ...
    const existingCartItem = cartData?.data.find(
      (item) => item.products[0].productId === id
    );
  
    if (existingCartItem) {
      const updatedData = cartData.data.map((item) =>
        item.products[0].productId === id
          ? {
              ...item,
              products: [
                {
                  ...item.products[0],
                  qty: item.products[0].qty + count,
                },
              ],
            }
          : item
      ...
      const updatedCart = { ...cartData, data: updatedData };
  ...
  useEffect(() => {
    handleAddDetails();
    // Assuming you have a function to get cart data
    const fetchCartData = async (id) => {
      const cartRes = await dispatch(getcartProduct(id));
      console.log("cartresponse getting inside",cartRes)
      if (cartRes) {
